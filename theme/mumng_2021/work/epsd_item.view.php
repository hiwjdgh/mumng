...
<li class="epsd-item epsd-detail <?=$epsd["is_hit"] ? "visit" : ""?>" data-id="<?=$epsd["work_id"]?>" data-epsd="<?=$epsd["epsd_id"]?>">
    <div class="epsd-item__img-box">
        <img src="<?=$epsd["epsd_img"]?>" alt="회차 이미지" width="222" height="222" class="epsd-item__img">
    ...
    <div class="epsd-item__info">
        ...
        <p class="epsd-item__info-upload">
            ...
            if($epsd["epsd_pay_info"] && !$epsd["epsd_pay_info"]["is_expired"]){
                echo Converter::display_time($epsd["epsd_upload_dt"]) ;
            }else{
                if($work["work_user"]["user_commerce_state"]){
                    if($work["work_free"]){
                        if($epsd["is_preview"]){
                            echo Converter::display_time($epsd["epsd_upload_dt"])."후 무료";
                        }else{
                            echo Converter::display_time($epsd["epsd_upload_dt"]) ;
                        }
                    }else{
                        if($epsd["is_preview"] && ($work["work_preview_exp"] > $work["work_rent_exp"])){
                            echo Converter::display_time($epsd["epsd_upload_dt"]). "후 ".Converter::display_number($work["work_rent_exp"])."EXP";
                        }else{
                            echo Converter::display_time($epsd["epsd_upload_dt"]) ;
                        }
                    }
        ...
        <div class="epsd-item__info-inner-box">
            <?php if($epsd["epsd_pay_info"] && !$epsd["epsd_pay_info"]["is_expired"]) {  ?>
                ...
                <?php } ?>
            <?php }else{ ?>
                <?php if($work["work_user"]["user_commerce_state"]){ ?>
