...
<section id="find" class="flex flex-column mxw-360 pdhz-22">
    <ul id="find__tab" class="flex gap-column-12 flex-align-self-flex-start mgb-48">
        <li class="text-weight-bold text-color-g5 text-size-large"><a href="<?=get_url(PAVE_ACCOUNT_URL,"find/id/form")?>" title="아이디 찾기">아이디 찾기</a></li>
        <li class="text-weight-bold text-color-g12 text-size-large"><a href="<?=get_url(PAVE_ACCOUNT_URL,"find/pwd/form")?>" title="비밀번호 찾기">비밀번호 찾기</a></li>
    </ul>
    <form id="find__form" class="flex flex-column" novalidate autocomplete="off">
        ...
        <fieldset class="flex flex-column mxw-316">
            <legend class="skip">비밀번호찾기</legend>

            <div class="input-box input-box-t2 mgb-10">
                <label for="user_id" class="input-box-t2__label">아이디</label>
                <input type="text" name="user_id" id="user_id" class="input-box-t2__input" required>
            ...
            <div class="input-box input-box-t2 mgb-60">
                ...
                <button type="button" id="user_cp_cert_button" class="cert-button input-box-t2__action" disabled data-cert="find_pwd">인증하기</button>
...
function find_step_check(){
    ...
    &&$("#user_cp").closest(".input-box").hasClass("complete")
...
$(document).ready(function(){
    $("#find__form").on("submit", async function(e){
        ...
        await check_find_pwd_form($(this));

        return false;
    });

    $("#user_id").on("keyup blur", async function(e){
        result = await check_user_id($(e.target).val(), false);
