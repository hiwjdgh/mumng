        ...
        <div class="page__left">
            <div class="page__card">
                    ...
                    <div class="helper__container">
                        <div id="helper__more-box" class="helper__action-box">
                            ...
                            <?php if($page_user["user_no"] == $pave_user["user_no"]){ ?>
                                ...
                                <a href="<?=get_url(PAVE_SETTING_URL, "profile")?>" class="helper__action-button" target="_blank">프로필 편집</a>
                            <?php }else{ ?>
                                <button type="button" class="helper__action-button penalty-button" style="color:#E34850;" data-type="user" data-target="<?=$page_user["user_no"]?>">작가 신고</button>
                                <?php if($page_user["user_follow"]["is_follow"]){ ?>
                                <button type="button" class="helper__action-button follow-button" data-user="<?=$page_user["user_no"]?>">팔로우 취소</button>
                                <?php }else{ ?>
                                <button type="button" class="helper__action-button follow-button" data-user="<?=$page_user["user_no"]?>">팔로우</button>
                                <?php } ?>
                            <?php } ?>
                        </div>
                ...
                <div class="page__card-follow-box">
                    <button type="button" class="page__card-follower-button follower-button" data-user="<?=$page_user["user_no"]?>">
                    ...
                    <button type="button" class="page__card-following-button following-button" data-user="<?=$page_user["user_no"]?>">
                    ...
                    <?php }else{ ?>
                        <?php if(User::is_follow_user($pave_user, $page_user)){ ?>
                        <button type="button" class="page__card-follow-button button-t3 button-s2 follow-button" data-user="<?=$page_user["user_no"]?>">팔로우 취소</button>
                        <?php }else{ ?>
                        <button type="button" class="page__card-follow-button button-t1 button-s2 follow-button" data-user="<?=$page_user["user_no"]?>">팔로우</button>
                        <?php } ?>
                    <?php } ?>
                </div>
                ...
                <?php if($page_user_major){ ?>
                <div class="page__card-major">
                    <h3 class="page__card-major-label">대표작품</h3>
                    <div class="page__card-major-box work-detail" data-id="<?=$page_user_major["work_id"]?>">
                        <img src="<?=$page_user_major["work_img"]?>" alt="대표작품 이미지" width="90" height="112" class="page__card-major-img">
                        <div class="page__card-major-inner-box">
                            <span class="page__card-major-epsd">총 <?=Converter::display_number($page_user_major["work_epsd_cnt"], "화") ?></span>
                            <span class="page__card-major-name text-truncate" ><?=$page_user_major["work_name"]?></span>
                            <p class="page__card-major-description text-truncate-line3"><?=$page_user_major["work_description"]?></p>
                        </div>
                    </div>
                </div>
                <?php } ?>
