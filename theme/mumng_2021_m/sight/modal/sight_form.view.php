...
<div id="modal" class="modals sight_form_modal" data-target="sight_form_modal">
    <div id="modal__box">
        <div id="modal__header" class="modal__header--lg">
            <h2 id="modal__header-title"><?=$sight_form_title?></h2>
            <button type="button" id="modal__header-close-button" class="modal-close-button" data-anchor="sight_form_modal"><span class="icon-x icon-16"></span><span class="skip">닫기</span></button>
        </div>
        <div id="modal__content">
            <div class="sight__box">
                <form id="sight__form" class="sight-form flex flex-column gap-row-16" enctype="multipart/form-data" method="POST" novalidate autocomplete="off">
                    <input type="hidden" name="csrf" id="csrf" value="<?=get_session("csrf_token")?>">
                    <input type="hidden" name="sight_id" id="sight_id" value="<?=$sight["sight_id"]?>">
                    <input type="hidden" name="action" id="action" value="<?=$action?>">


                    <fieldset class="flex gap-column-16">
                    ...
                    <fieldset class="flex gap-column-24 sight__fieldset">
                        <legend class="skip">창작물 정보</legend>

                        <div class="sight__info flex flex-column gap-row-16 mxw-360">
                            ...
                            <div class="flex flex-column gap-row-12">
                                ...
                                <div class="file-sight <?=$sight["sight_img"] ? "edit" : ""?>">
                            ...
                            <div class="flex flex-column gap-row-12">
                                <div class="flex gap-column-6">
                                    ...
                                    <span class="text-size-normal text-color-g7 text-weight-medium sight-hashtag-counter" data-max="<?=$sight_config["sight_hashtag_max_cnt"]?>">0/<?=$sight_config["sight_hashtag_max_cnt"]?></span>
                                </div>
                                ...
                                <div class="flex flex-wrap gap-6 bd-1-solid-g4 pd-10 bdrd-20 sight-hashtag-box"></div>
                            </div>
                        </div>

                        <div class="flex flex-column flex-auto gap-row-24">
                            <div class="flex flex-column gap-row-12">
                                <h3 class="text-size-normal text-color-g10 text-weight-medium">내용</h3>
                                <textarea name="sight_content" id="pave-editor" cols="30" rows="10" style="width: 100%;"></textarea>
                            </div>


                            <div class="flex gap-24 flex-align-self-flex-end">
                                <button type="button" class="text-size-lgx4 text-color-g8 text-weight-bold modal-close-button">취소</button>
                                <button type="submit" class="text-size-lgx4 text-color-g11 text-weight-bold">등록</button>
                            </div>
                        </div>
                    </fieldset>
                </form>
